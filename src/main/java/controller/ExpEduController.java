...
public class ExpEduController extends PController<ExpEdu, ExpEduService> {
          ...
          @PostMapping("/crear")
          public ResponseEntity<Response> crear(@RequestBody ExpEdu obj, @RequestHeader String Authorization) {
              ...
              @PutMapping("/editar")
          public ResponseEntity<String> editar(@RequestBody ExpEdu obj, @RequestHeader String Authorization) {
                    Usuario user = serv.traerPorEmail(JwtUtil.getEmail(Authorization.replace("Bearer ","")));
                    obj.setUser(user);
                    if (inter.editar(obj)) {
